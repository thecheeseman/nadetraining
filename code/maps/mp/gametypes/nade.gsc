...
Callback_PlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc)
{
	...
	if ( isdefined( self.isfollowingnade ) )
		return;
...
// This is a really hacky way of doing it
// but it works :)
nade()
{
	self endon( "death" );
...
marker()
{
	...
	while ( 1 )
	{
		if ( self usebuttonpressed() && self meleebuttonpressed() && !isdefined( self.placedmarker ) )
		{
			time = 0.00;
			...
			self.placedmarker = true;
			objective_add( level.markers, "current", self.origin, "gfx/hud/hud@objective.tga" );		
			self thread markerstop( level.markers );
			level.markers++;
			
			self playsound( "explo_plant_no_tick" );
			
			iprintln( self.name + "^7 placed a marker." );
